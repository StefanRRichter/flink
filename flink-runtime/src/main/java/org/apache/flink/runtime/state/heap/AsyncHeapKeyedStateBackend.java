...
public class AsyncHeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
	...
	private <N, V> StateTable<K, N, V> tryRegisterStateTable(
		...
		if (stateTable == null) {
			stateTable = new StateTable<>(this, newMetaInfo);
	...
	public  RunnableFuture<KeyGroupsStateHandle> snapshot(
		...
		final Map<AbstractStateTable<K, ?, ?>, StateTableSnapshot<K, ?, ?>> cowStateStableSnapshots = new HashedMap(stateTables.size());
		...
		final AbstractAsyncIOCallable<KeyGroupsStateHandle, CheckpointStreamFactory.CheckpointStateOutputStream> ioCallable =
				new AbstractAsyncIOCallable<KeyGroupsStateHandle, CheckpointStreamFactory.CheckpointStateOutputStream>() {
					...
					public void done(boolean canceled) {
						...
						for (Map.Entry<AbstractStateTable<K, ?, ?>, StateTableSnapshot<K, ?, ?>> entry
			...
			try {
				...
				for (KeyedBackendSerializationProxy.StateMetaInfo<?, ?> metaInfoSerializationProxy : metaInfoList) {
					...
					if (null == stateTable) {
						...
						stateTable = new StateTable<>(this, registeredBackendStateMetaInfo);
	...
	private <N, S> void readStateTableForKeyGroup(
			...
			AbstractStateTable<K, N, S> stateTable) throws IOException {
	...
	public int numStateEntries(Object namespace) {
		int sum = 0;
		for (AbstractStateTable<K, ?, ?> stateTable : stateTables.values()) {
			sum += stateTable.numberOfKeysInNamespace(namespace);
		}
		return sum;
	}
}