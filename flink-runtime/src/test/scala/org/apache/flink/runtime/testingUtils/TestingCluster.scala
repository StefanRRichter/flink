...
class TestingCluster(
  ...
  def requestCheckpoint(jobId: JobID, options : CheckpointOptions): String = {
    ...
    val allRunning = jobManagerGateway.ask(TestingJobManagerMessages.WaitForAllVerticesToBeRunning(jobId), timeout)
    ...
    result match {
      ...
      case fail: CheckpointRequestFailure => throw new IOException(fail.cause)
  ...
  def cancelJob(jobId: JobID): Unit = {
    if (getCurrentlyRunningJobsJava.contains(jobId)) {
      ...
      if (!result.isInstanceOf[JobManagerMessages.CancellationSuccess]) throw new Exception("Cancellation failed")
    }
