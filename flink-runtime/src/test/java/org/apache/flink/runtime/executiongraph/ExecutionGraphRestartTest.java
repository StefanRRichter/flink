...
public class ExecutionGraphRestartTest extends TestLogger {
	...
	public void testConcurrentGlobalFailAndRestarts() throws Exception {
		...
		for (ExecutionVertex v : testMainThread.execute(eg::getAllExecutionVertices)) {
			testMainThread.execute(() -> v.getCurrentExecutionAttempt().cancelingComplete());
		}
		...
		testMainThread.execute(restartTrigger::trigger);
	...
	private void restartAfterFailure(ExecutionGraph eg, FiniteDuration timeout, boolean haltAfterRestart) throws InterruptedException, TimeoutException {
		testMainThread.execute(() -> ExecutionGraphTestUtils.failExecutionGraph(eg, new Exception("Test Exception")));

		// Wait for async restart
		testMainThread.execute(() -> waitForAsyncRestart(eg, timeout));
